 ...
 */
/**
 * **GUM が動くのは「倒しきった束の数」だけです。倒した敵の数では動きません。**
 *
 * 黒ウィズ型リデザインで、**速く答えるほど多くのASが乗る**ようになりました
 * （EXCELLENT 5体 / NICE 2体）。速いほど敵を早く倒し、次の敵が湧き、
 * 倒した数が増えます。**そこに GUM を付けると、答える速さが持ち物の差に
 * なります**——原則3-2（報酬をプレイスタイルで差別化しない）と正面からぶつかります。
 *
 * **倒しきった束の数は速さで増えません。** 束は3つと決まっていて、敵の体力は
 * 「NICE でその束の8割に正解したら倒れる」ところに置いてあるので、
 * 速く答えられない人も同じだけ倒しきれます。**だから GUM はこちらにだけ付けます。**
 *
 * 倒した数のほうは**点だけ**を動かします。点は買えるものが何も無く、称号の条件にも
 * 入っていないので、持ち物の差になりません（「点数だけは例外です」）。
 */
export const KILL_GUM = 0;      // 倒した敵の数では GUM は動かない（速さで増えるため）
export const CLEAR_GUM = 5;     // wave の終わりに敵が残っていなければ
